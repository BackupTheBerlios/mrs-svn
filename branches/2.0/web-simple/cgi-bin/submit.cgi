...
my $blastmat = "/usr/local/ncbi/data/";
my $ncbi = "/usr/local/ncbi/";
my $work_dir = "/usr/tmp/blast/";

my $base_url = $q->url({-absolute=>1});
$base_url =~ s|/cgi-bin/submit(\S*)\.cgi$||;
$base_url =~ s|/$||;
...
sub submitJobs()
{
	...
	foreach my $f (@queries)
	{
		...
		print SCRIPT "( /usr/bin/env BLASTDB=$blastdir BLASTMAT=$blastmat NCBI=$ncbi $ncbi/bin/blastall -p $program $options -i $query_name.fa > $query_name.out ) >& $query_name.err\n";
		close SCRIPT;
		...
		my $submission_id = `. /usr/sge/default/common/settings.sh; /usr/sge/bin/glinux/qsub /$work_dir/$query_name.csh`;
#		
